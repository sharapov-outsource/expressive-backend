...
class DoctrineObject extends AbstractHydrator
{
    ...
    protected $defaultByValueStrategy
        = __NAMESPACE__ . '\Strategy\AllowRemoveByValue';
    /**
     * @var string
     */
    protected $defaultByReferenceStrategy
        = __NAMESPACE__ . '\Strategy\AllowRemoveByReference';
    ...
    {
        $this->metadata
            = $this->objectManager->getClassMetadata(get_class($object));
                ...
                if ($metadata->isSingleValuedAssociation($field)) {
                    ...
                    if (null === $value
                        && ! current($metadata->getReflectionClass()
                            ->getMethod($setter)->getParameters())->allowsNull()
                    ) {
                        continue;
                    }
                    $object->$setter($value);
    ...
    protected function toOne($target, $value)
    {
        ...
        if (is_array($value)
            && array_keys($value) != $metadata->getIdentifier()
        ) {
    ...
    public function hydrateValue($name, $value, $data = null)
    {
        ...
        return $this->handleTypeConversions(
            $value,
            $this->metadata->getTypeOfField($name)
        );
    }
    ...
    private function isNullable($name)
    {
        ...
        if ($this->metadata->hasField($name)) {
            return method_exists($this->metadata, 'isNullable')
                && $this->metadata->isNullable($name);
        } else {
            if ($this->metadata->hasAssociation($name)
                && method_exists($this->metadata, 'getAssociationMapping')
            ) {
                $mapping = $this->metadata->getAssociationMapping($name);
                return false !== $mapping && isset($mapping['nullable'])
                    && $mapping['nullable'];
            }
        }
        return false;
    ...
    {
        $metadata = $this->objectManager->getClassMetadata(ltrim(
            $target,
            '\\'
        ));
        ...
        foreach ($values as $value) {
            ...
            if (! empty($find) && $found = $this->find($find, $target)) {
                $collection[] = (is_array($value)) ? $this->hydrate(
                    $value,
                    $found
                ) : $found;
            } else {
